...
public:
	...
	{
		std::ofstream bmp("./ptcs.bmp", std::ios::binary);
		BmpHeader header;
		bmp.write("BM", 2);
...
private:
		...
		float          *pdf) const
	{
		...
		*pdf += probs.diffProb * std::max(0.f, Z * INV_PI_F);
		//cout << "DLz: " << directionL.z << endl;
		return mat.diffuseFact * INV_PI_F;
	}
		...
		float          *pdf) const
	{
		...
		float dot_R_Wi = Dot(reflLocalDirIn, directionL);
